...
import { managementCodeExample } from '../data/managementCodes';
...
export function RecordFormScreen({ category, title, messages, record, onBack, onSaved }: Props) {
  ...
  const save = async () => {
    if (!name.trim()) { Alert.alert('Tên là thông tin bắt buộc.'); return; }
    setSaving(true);
    try {
  ...
  const fields = [
    ...
    ...(inventory ? [{ label: messages.common.stock, value: quantity, setValue: setQuantity, placeholder: '0' }, { label: 'Đơn vị', value: unit, setValue: setUnit, placeholder: 'EA' }] : []),
  ];
  ...
  return (
    <View style={styles.page}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.subtitle}>Mã quản lý được hệ thống cấp tự động và không thay đổi trong suốt vòng đời hồ sơ.</Text>
        <View style={styles.live}><View style={styles.dot} /><Text style={styles.liveText}>SUPABASE · {record ? 'CẬP NHẬT' : 'TẠO MỚI'}</Text></View>

        <View style={styles.codeCard}>
          <View style={styles.codeIcon}><Ionicons name="lock-closed-outline" size={20} color={colors.primary} /></View>
          <View style={styles.codeBody}>
            <Text style={styles.codeLabel}>Mã quản lý</Text>
            <Text style={styles.codeValue}>{record ? code : `Tự động cấp khi lưu · Ví dụ ${codeExample}`}</Text>
            <Text style={styles.codeHint}>Không nhập tay · Không đổi mã · Không tái sử dụng mã cũ</Text>
          </View>
        </View>
