...
type Props = { category: CatalogKey; title: string; messages: any; record?: MasterRecord; onBack: () => void; onSaved: (record: MasterRecord) => void };
...
export function RecordFormScreen({ category, title, messages, record, onBack, onSaved }: Props) {
  ...
  const chooseGroup = (id: string) => {
    setGroupId(id);
    if (category === 'assets' && id !== record?.groupId) setTypeId('');
  };
  ...
  return (
    <View style={styles.page}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.subtitle}>{category === 'assetTypes' ? 'Loại tài sản là dữ liệu cấu hình; có thể bổ sung mà không phải sửa chương trình.' : 'Mã quản lý được hệ thống cấp tự động và không thay đổi trong suốt vòng đời hồ sơ.'}</Text>
