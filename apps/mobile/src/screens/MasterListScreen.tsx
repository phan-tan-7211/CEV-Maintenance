...
type Props = { category: CatalogKey; title: string; messages: any; onBack: () => void; onOpenRecord: (record: MasterRecord) => void; onCreate: () => void };
...
export function MasterListScreen({ category, title, messages, onBack, onOpenRecord, onCreate }: Props) {
  ...
  const rows = useMemo(() => {
    ...
    if (!q) return records;
    return records.filter((r) => `${r.code} ${r.name} ${r.secondary} ${r.location ?? ''} ${r.group ?? ''} ${r.type ?? ''} ${r.parentCode ?? ''} ${r.linkedAssetCode ?? ''}`.toLowerCase().includes(q));
  }, [records, query]);

  const statusLabel = (status: MasterRecord['status']) => status === 'active' ? messages.common.active : status === 'warning' ? messages.common.warning : messages.common.inactive;

  return (
    <View style={styles.page}>
      <View style={styles.header}><TouchableOpacity onPress={onBack} style={styles.back}><Ionicons name="chevron-back" size={24} color={colors.text} /><Text style={styles.backText}>{messages.nav.catalog}</Text></TouchableOpacity><TouchableOpacity onPress={onCreate} style={styles.add}><Ionicons name="add" size={22} color="#fff" /><Text style={styles.addText}>{messages.common.add}</Text></TouchableOpacity></View>
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void load(true)} />}>
        <Text style={styles.title}>{title}</Text><Text style={styles.subtitle}>{messages.common.masterListSubtitle}</Text>
        <View style={styles.live}><View style={styles.dot} /><Text style={styles.liveText}>CƠ SỞ DỮ LIỆU · TRỰC TIẾP</Text></View>
        ...
        {!loading ? <View style={styles.list}>{rows.map((item) => (
          <TouchableOpacity key={item.id} style={styles.card} activeOpacity={0.72} onPress={() => onOpenRecord(item)}>
            <View style={styles.iconWrap}><Ionicons name={category === 'assetTypes' ? 'git-branch-outline' : 'cube-outline'} size={21} color={colors.primary} /></View>
            <View style={styles.body}>
              <View style={styles.codeRow}>{item.code ? <Text style={styles.code}>{item.code}</Text> : <Text style={styles.code}>{item.group ?? 'LOẠI TÀI SẢN'}</Text>}<View style={[styles.status, item.status === 'warning' && styles.statusWarn]}><Text style={[styles.statusText, item.status === 'warning' && styles.statusWarnText]}>{statusLabel(item.status)}</Text></View></View>
              <Text style={styles.name}>{item.name}</Text>
              {item.group || item.type ? <Text style={styles.classification}>{[item.group, item.type].filter(Boolean).join(' › ')}</Text> : null}
              <Text style={styles.meta}>{item.secondary}{item.location ? ` · ${item.location}` : ''}</Text>
              {item.parentCode ? <Text style={styles.due}>Thuộc: {item.parentCode}</Text> : null}
              {item.linkedAssetCode ? <Text style={styles.due}>Tài sản: {item.linkedAssetCode}</Text> : null}
